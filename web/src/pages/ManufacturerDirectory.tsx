      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 flex items-center gap-2">
            <Building2 className="w-6 h-6 text-nyaya-700" /> Manufacturer Directory
          </h1>
          <p className="text-slate-600">Search manufacturers and view aggregated compliance history.</p>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {filtered.map(mfr => (
                <TableRow key={mfr.id}>
                  <TableCell>
                    <div className="font-medium text-slate-900">{mfr.name}</div>
                    <div className="text-xs text-slate-500">{mfr.id}</div>
                  </TableCell>
                  <TableCell className="text-sm text-slate-700">{mfr.hq}</TableCell>
                  <TableCell className="text-center text-sm font-medium text-slate-900">{mfr.inspections}</TableCell>
                  <TableCell className="text-center">
                    <div className={`text-sm font-semibold ${mfr.complianceRate < 80 ? 'text-red-600' : 'text-emerald-600'}`}>
                      {mfr.complianceRate}%
                    </div>
                  </TableCell>
                  <TableCell>
                    {mfr.repeatOffender ? (
                      <div className="flex items-center text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-1 rounded w-fit">
